...
// [FIX 1] : Matikan batasan memory & waktu untuk data banyak
ini_set('memory_limit', '-1'); 
set_time_limit(0); 

// [FIX 2] : Pastikan error reporting mati di output JSON
error_reporting(0);
...
// [FIX 3] : Fungsi Helper untuk nilai SQL agar aman di Linux (Strict Mode)
...
function hitungPensiun($tgl_lahir, $tgl_pensiun_input = '') {
    ...
    if (!empty($lahir) && $lahir != '1970-01-01') {
        try {
            ...
            $date->modify('+58 years'); // Update: Usia pensiun umum biasanya 58/60
...
try {
    ...
    if ($action === 'preview') {
        ...
        try {
            ...
            throw new Exception("Gagal membaca file Excel. File mungkin corrupt.");
        }
        ...
        // [FIX 4] : Kurangi beban DOM browser jika data ribuan
        $preview_rows = array_slice($rows, 0, 15);
        ...
        foreach ($preview_rows as $row) {
            ...
            if(!empty($id_excel)) {
                ...
                if(mysqli_num_rows($cekId) > 0) {
                    ...
                    if(mysqli_num_rows($cekNama) > 0) $status_row = '<span class="badge badge-danger">Ganti ID (Nama Cocok)</span>';
                }
            }
            ...
            foreach ($row as $index => $cell) {
                if ($index == 4 || $index == 14) { 
                    ...
                    $html .= '<td>' . ($tgl ? $tgl : '-') . '</td>';
                     ...
                     // Preview hitung pensiun
                    $pensiun = hitungPensiun(isset($row[4])?$row[4]:'', $cell);
                    $html .= '<td>' . ($pensiun ? $pensiun : '-') . '</td>';
                } else {
        ...
        if (count($rows) > 15) {
            $html .= '<div class="alert alert-info py-2 my-2"><i class="fas fa-info-circle"></i> Menampilkan 15 dari '.count($rows).' data. Total data akan diproses saat disimpan.</div>';
        }
    ...
    // --- MODE SAVE ---
    elseif ($action === 'save') {
        // [FIX 5] : Validasi Data kosong
        if (!isset($_POST['data_pegawai']) || empty($_POST['data_pegawai'])) {
             throw new Exception("Data tidak diterima. Kemungkinan file terlalu besar (Max POST Size Server).");
        }
        
        $data = json_decode($_POST['data_pegawai'], true);
        if (!$data) throw new Exception("Format data corrupt atau terlalu besar.");
        ...
        foreach ($data as $row) {
            ...
            // --- DATA PREPARATION (Strict Mode Safe) ---
            $v_id_peg   = mysqli_real_escape_string($conn, $id_peg_raw);
            $v_nip      = isset($row[1]) ? trim($row[1]) : ''; // Jangan di escape dulu, biar helper yang handle
            ...
            // Gunakan Helper getSqlVal untuk membungkus nilai
            // Jika kosong -> return "NULL", Jika isi -> return "'Isi'"
            ...
            $sql_bpjstk       = getSqlVal($conn, isset($row[16]) ? $row[16] : '', 'int'); // Mode INT
            $sql_bpjskes      = getSqlVal($conn, isset($row[17]) ? $row[17] : '', 'int'); // Mode INT

            // Handle NIP (kadang dianggap string kadang int, kita perlakukan sebagai string angka)
        ...
        echo json_encode([
            ...
            'message' => "<b>Import Selesai!</b><br>
                          Input Baru: $berhasil<br>Update: $updated<br>Ganti ID: $updated_id<br>Gagal: $gagal"
        ]);
        exit;
    }
