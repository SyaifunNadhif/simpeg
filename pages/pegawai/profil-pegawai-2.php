            ...
            <div class="card card-primary card-outline">
              <div class="card-body box-profile">
                <div class="text-center">
					...
					<?php
					// ==== Resolver path foto yang tahan banting ====
					$raw      = trim((string)($peg['foto'] ?? ''));
					$jk       = strtolower(trim((string)($peg['jk'] ?? '')));

					// Basis filesystem dari file yang sedang dieksekusi (home-admin.php di root)
					$FS_ROOT  = rtrim(str_replace('\\','/', dirname($_SERVER['SCRIPT_FILENAME'])), '/'); // .../htdocs/yourapp
					// Basis URL relatif dari root web (karena home-admin.php di root)
					$URL_BASE = ''; // kosongkan karena src di bawah sudah relatif dari root app

					// 1) Jika DB sudah menyimpan path lengkap relatif web (mis. "pages/asset/foto/x.jpg")
					$candidates = [];
					if ($raw !== '') {
						$rawClean = ltrim($raw, '/');
						// jika sudah path, coba langsung
						$candidates[] = $rawClean;
						// jika hanya filename, coba di semua folder yang mungkin
						$file = basename($rawClean);
						$candidates[] = "uploads/foto/$file";
						$candidates[] = "pages/assets/foto/$file"; // plural
						$candidates[] = "pages/asset/foto/$file";  // singular (legacy)
					}

					// 2) Fallback default berdasarkan jk
					...
					// Pilih yang benar-benar ada di filesystem
					$src = null;
					foreach ($candidates as $rel) {
						$fs = "$FS_ROOT/$rel";
						if (file_exists($fs)) { $src = $rel; break; }
					}
					if ($src === null) $src = "pages/assets/foto/no-foto.png"; // terakhir banget

					// cache-buster: agar update foto langsung tampil
					$cb = time();
					?>
					<img
