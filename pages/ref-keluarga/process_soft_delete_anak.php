<?php
/*********************************************************
 * FILE     : pages/ref-keluarga/process_soft_delete_anak.php
 * MODULE   : Backend Delete Data Anak (Secure Transaction)
 *********************************************************/
...
if ($action === 'get_info') {
  ...
  if ($stmt->execute()) {
      $stmt->bind_result($nama_anak, $nama_peg);
      if ($stmt->fetch()) {
        $stmt->close();
...
if ($action === 'delete') {
  ...
  try {
    ...
    $stmtSel = $conn->prepare("SELECT * FROM tb_anak WHERE id_anak = ? LIMIT 1");
    ...
    $stmtDel = $conn->prepare("DELETE FROM tb_anak WHERE id_anak = ?");
