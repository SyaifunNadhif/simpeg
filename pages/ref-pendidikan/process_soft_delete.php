<?php
/*********************************************************
 * FILE     : pages/ref-pendidikan/process_soft_delete.php
 * MODULE   : Backend Delete Pendidikan (Secure Transaction)
 *********************************************************/
...
if ($action === 'get_info') {
  // Ambil data untuk ditampilkan "Anda akan menghapus data X..."
  $sql = "SELECT edu.jenjang, edu.jurusan, edu.nama_sekolah, p.nama AS nama_peg
          FROM tb_pendidikan edu
          LEFT JOIN tb_pegawai p ON edu.id_peg = p.id_peg
          WHERE edu.id_pendidikan = ?"; 
  ...
  if ($stmt->execute()) {
      $stmt->bind_result($jenjang, $jurusan, $sekolah, $nama_peg);
      if ($stmt->fetch()) {
        $stmt->close();
...
if ($action === 'delete') {
  ...
  try {
    ...
    $stmtSel = $conn->prepare("SELECT * FROM tb_pendidikan WHERE id_pendidikan = ? LIMIT 1");
    ...
    $stmtDel = $conn->prepare("DELETE FROM tb_pendidikan WHERE id_pendidikan = ?");
